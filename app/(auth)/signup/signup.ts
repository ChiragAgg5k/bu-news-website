...
function writeUserData(userId: string, name: string, phoneNo: string, city: string, admin: boolean) {
...
export async function Signup(props: SignUpPageProps) {
	...
	try {
		...
		writeUserData(result.user.uid, props.name, props.phoneNo, props.city, props.admin);
	} catch (e) {
		error = e;
	}
